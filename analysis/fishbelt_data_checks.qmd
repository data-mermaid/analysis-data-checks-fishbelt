...
cat("**Project analyzed:**", targetProject$project[1], "\n\n")
cat("**Project ID:**", targetProject$project_id[1], "\n\n")
if (!is.na(targetProject$tags[1])) {
  cat("**Tags:**", targetProject$tags[1], "\n\n")
}
if (!is.na(targetProject$project_notes[1])) {
  cat("**Project notes:**", targetProject$project_notes[1], "\n\n")
}

# Print data summary
cat("Data extracted:\n")
cat("- Observations:", nrow(observations_data), "records\n")
cat("- Sample Units:", nrow(sampleunits_data), "transects\n")
cat("- Sample Events:", nrow(sampleevents_data), "sites\n")
```
...
cat("Sample Coverage:\n")
cat("- Total sample events:", nrow(sampleevents_data), "\n")
cat("- Total transects:", nrow(sampleunits_data), "\n")
cat("- Total observations:", nrow(observations_data), "\n")
cat("- Unique sites:", length(unique(sampleevents_data$site)), "\n")
cat("- Unique observers:", 
    length(unique(sampleevents_data$observers)), "\n\n")

cat("Taxonomic Diversity:\n")
cat("- Fish families:", n_families, "\n")
cat("- Fish genera:", length(unique(observations_data$fish_genus)), "\n")
cat("- Fish species:", length(unique(observations_data$fish_taxon)), "\n\n")

cat("Biomass Statistics (kg/ha):\n")
cat("- Mean:", round(mean(sampleevents_data$biomass_kgha_avg, 
                         na.rm = TRUE), 1), "\n")
cat("- Median:", round(median(sampleevents_data$biomass_kgha_avg, 
                           na.rm = TRUE), 1), "\n")
cat("- SD:", round(sd(sampleevents_data$biomass_kgha_avg, 
                      na.rm = TRUE), 1), "\n")
cat("- Range:", round(min(sampleevents_data$biomass_kgha_avg, 
                          na.rm = TRUE), 1), "-",
    round(max(sampleevents_data$biomass_kgha_avg, na.rm = TRUE), 1), "\n\n")

cat("Data Quality Flags:\n")
cat("- High biomass outliers (>90th percentile):", 
    nrow(high_biomass_sites), "\n")
cat("- Statistical outliers (IQR method):", nrow(outliers), "\n")
cat("- Sites with shark observations:", 
